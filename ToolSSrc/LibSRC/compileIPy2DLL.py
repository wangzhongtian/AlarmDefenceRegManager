...
def compileDirs2AppDLL( paths=[],outname ="AppAssembly"):
	...
	with codecs.open("resp.txt","wt",encoding="utf-8" )  as f1:
		for i in gb1:
			print(i)
			# f1.write( i );
			# f1.write("\n")
	...
	cmdTpl ="ipy2.7.exe pyc.py   /out:{}  {}" 	 
	# cmdTpl ="ipyc2.7.exe   /out:{}  {}" 
	# _srcfiles = "@resp.txt"
	cmdStr =  cmdTpl.format( os.path.join(libpath,outname),_srcfiles   )
	print( "will execute:",cmdStr )
	os.system( cmdStr )
def mainLibComp():
	curdir =os.getcwd()
	# libpath=os.path.dirname( __file__ )
